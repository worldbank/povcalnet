...
{phang2}
{dlgtab: 5. Calculate regional and global aggregates using lined-up country data }
...
{cmd}
	...
	.	merge 1:1 countrycode year using `all', keep(1 3) nogen
	...
	.	merge m:1 countrycode using `regions', nogen  //merge regioncodes
	.	keep regioncode countrycode year headcount population  //merge regioncodes
	...
	.	br if year==2017
	.	tempfile aggregates
	.	save `aggregates', replace // Global and Regional Poverty Rates

	.	import excel using "https://databank.worldbank.org/data/download/site-content/OGHIST.xls", clear sheet ("Country Analytical History") cellrange(A5:AI240) firstrow //income group classification data
	.	rename A code
...
replace `var' = "UM" if inlist(code,"HRV","CZE","SVK")
	...
	.	drop if code=="CSK"
	.	drop if inlist(code,"MYT","ANT","SUN")// Dropping three economies that are not among the WB's 218 economies
...
local year = `year' + 1
	...
	.	use CLASS.dta, clear	//merge income classification
	.	keep if year==`y'
	.	merge 1:m countrycode using `surveyyears' ,nogen //three years window
	.	replace year=`y' if year==.
	.	gen covered = (abs(year-datayear)<=3)
	.	keep if covered==1 //keep countries with a survey within three years window
	...
	.	use CLASS.dta, clear
	.	keep if year==`y'
	.	merge 1:m countrycode year using `population', keep (1 3) nogen 
	...
	.	save    `LICLMIC'
	.	restore
	.	collapse covered [aw=pop], by(year regioncode) // Regional coverage
	.	append using `world'
